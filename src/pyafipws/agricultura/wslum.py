...
class WSLUM(BaseWS):
    ...
    def __analizar_errores(self, ret):
        ...
        if errores:
            self.Errores = [
                "{codigo}: {descripcion}".format(**err["error"][0]) for err in errores
            ]
            self.errores = [
                {
        ...
        if sep is None:
            ...
            for it in array:
                for subtipo in it["subtipo"]:
                    ret.append(
                        (
                            f"{sep} %s {sep} %s {sep} %s {sep} %s {sep} %s {sep} %s {sep}"
                        )
                        % (
                            it["codigo"],
                            it["descripcion"],
...
def main():
    ...
    try:
        ...
        config.get("WSLUM", "ENTRADA")
        config.get("WSLUM", "SALIDA")
        ...
        if "--autorizar" in sys.argv:
            ...
            print(
                "Liquidacion: pto_vta={} nro_cbte={} tipo_cbte={}".format(
        ...
        if "--consultar" in sys.argv:
            ...
            print(
                f"Consultando: tipo_cbte={tipo_cbte} pto_vta={pto_vta} nro_cbte={nro_cbte}"
            )
        ...
        if "--ult" in sys.argv:
            ...
            print(f"Consultando ultimo nro_cbte para pto_vta={pto_vta}", end=" ")
