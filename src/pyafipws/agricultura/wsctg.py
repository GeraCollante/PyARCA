...
ENCABEZADO = [
    ...
    ("vigencia_hasta", 10, A),
    ("transaccion", 12, N),
    ("tarifa_referencia", 6, IMPORTE, 2),  # consultar detalle
    ("estado", 20, A),
...
class WSCTG(BaseWS):
    ...
    def __analizar_controles(self, ret):
        ...
        if "arrayControles" in ret:
            ...
            self.Controles = [
                "{tipo}: {descripcion}".format(**ctl["control"]) for ctl in controles
            ]
    ...
    def ConsultarEstablecimientos(self, sep="||"):
        ...
        return [("{}".format(it["establecimiento"])) for it in array]
...
def leer_archivo(nombre_archivo):
    ...
    if ext == ".csv":
        csv_reader = csv.reader(open(nombre_archivo), dialect="excel", delimiter=";")
        ...
        raise RuntimeError(f"Extension de archivo desconocida: {ext}")
    archivo.close()
    ...
    try:
        ...
        if "--solicitar" in sys.argv:
            ...
            for it in items:
                print(
                    "solicitando...",
                    " ".join([f"{k}={v}" for k, v in list(it.items())]),
                )
        ...
        if "--parcial" in sys.argv:
            ...
            for it in items:
                print(
                    ...
                    " ".join([f"{k}={v}" for k, v in list(parcial.items())]),
                )
        ...
        if "--consultar_constancia_pdf" in sys.argv:
            ...
            wsctg.LanzarExcepciones = True
            wsctg.ConsultarConstanciaCTGPDF(ctg, archivo)
            print("Errores:", wsctg.Errores)
