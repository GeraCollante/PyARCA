...
class FEPDF:
    ...
    monedas_ds = {
        "DOL": "USD: Dólar",
        "PES": "ARS: Pesos",
        "010": "MXN: Pesos Mejicanos",
        "011": "UYU: Pesos Uruguayos",
        "012": "BRL: Real",
        "014": "Coronas Danesas",
        "015": "Coronas Noruegas",
        "016": "Coronas Suecas",
        "019": "JPY: Yens",
        "018": "CAD: D\xf3lar Canadiense",
        "033": "CLP: Peso Chileno",
        "056": "Forint (Hungr\xeda)",
        "031": "BOV: Peso Boliviano",
        "036": "Sucre Ecuatoriano",
        "051": "D\xf3lar de Hong Kong",
        "034": "Rand Sudafricano",
        "053": "D\xf3lar de Jamaica",
        "057": "Baht (Tailandia)",
        "043": "Balboas Paname\xf1as",
        "042": "Peso Dominicano",
        "052": "D\xf3lar de Singapur",
        "032": "Peso Colombiano",
        "035": "Nuevo Sol Peruano",
        "061": "Zloty Polaco",
        "060": "EUR: Euro",
        "063": "Lempira Hondure\xf1a",
        "062": "Rupia Hind\xfa",
        "064": "Yuan (Rep. Pop. China)",
        "009": "Franco Suizo",
        "025": "Dinar Yugoslavo",
        "002": "USD: D\xf3lar Libre EEUU",
        "027": "Dracma Griego",
        "026": "D\xf3lar Australiano",
        "007": "Florines Holandeses",
        "023": "VEB: Bol\xedvar Venezolano",
        "047": "Riyal Saudita",
        "046": "Libra Egipcia",
        "045": "Dirham Marroqu\xed",
        "044": "C\xf3rdoba Nicarag\xfcense",
        "029": "G\xfcaran\xed",
        "028": "Flor\xedn (Antillas Holandesas)",
        "054": "D\xf3lar de Taiwan",
        "040": "Lei Rumano",
        "024": "Corona Checa",
        "030": "Shekel (Israel)",
        "021": "Libra Esterlina",
    ...
    tipos_fact = {
        ...
        (201, 206, 211): "Factura de Crédito electrónica MiPyMEs (FCE)",
        (202, 207, 212): "Nota de Débito electrónica MiPyMEs (FCE)",
        (203, 208, 213): "Nota de Crédito electrónica MiPyMEs (FCE)",
        (4, 9, 15, 54): "Recibo",
    ...
    def fmt_cuit(self, c):
        if c is not None and str(c):
            c = str(c)
            return len(c) == 11 and f"{c[0:2]}-{c[2:10]}-{c[10:]}" or c
        return ""
    ...
    def CrearPlantilla(self, papel="A4", orientacion="portrait"):
        ...
        t = Template(
            ...
            title="{} {} {}".format(tipo.encode("latin1", "ignore"), letra, nro),
            author=f"CUIT {self.CUIT}",
            subject="CAE {}".format(fact["cae"]),
            keywords="AFIP Factura Electrónica",
            creator=f"PyFEPDF {__version__} (http://www.PyAfipWs.com.ar)",
        )
        self.template = t
        return True
    ...
    def ProcesarPlantilla(self, num_copias=3, lineas_max=36, qty_pos="izq"):
        ...
        try:
            ...
            permisos = [
                "Codigo de Despacho {} - Destino de la mercadería: {}".format(p["id_permiso"], self.paises.get(p["dst_merc"], p["dst_merc"]))
            ...
            cmps_asoc = [
                "{} {} {}".format(*self.fmt_fact(c["cbte_tipo"], c["cbte_punto_vta"], c["cbte_nro"]))
            ...
            if fact.get("motivos_obs") and fact["motivos_obs"] != "00":
                if not f.has_key("motivos_ds.L"):
                    motivos_ds = (
                        "Irregularidades observadas por AFIP (F136): {}".format(fact["motivos_obs"])
                    )
                else:
                    motivos_ds = "{}".format(fact["motivos_obs"])
            elif HOMO:
            ...
            for copia in range(1, num_copias + 1):
                ...
                for hoja in range(1, hojas + 1):
                    ...
                    f.set("pagina", f"Pagina {hoja} de {hojas}")
                    ...
                    if DEBUG:
                        print(f"generando pagina {hoja} de {hojas}")
                    ...
                    f.set(
                        "Cliente.TipoDoc",
                        "{}:".format(self.tipos_doc[int(str(fact["tipo_doc"]))]),
                    )
                    ...
                    for it in li_items:
                        ...
                        if it["importe"]:
                            subtotal += Decimal("{:.6f}".format(float(it["importe"])))
                            if letra_fact in ("A", "M") and it["imp_iva"]:
                                subtotal -= Decimal("{:.6f}".format(float(it["imp_iva"])))
                    ...
                    if hojas == hoja:
                        ...
                        subtotal = Decimal(0)
                        if fact["imp_neto"]:
                            subtotal += Decimal("{:.6f}".format(float(fact["imp_neto"])))
                        # agregar IVA al subtotal si no es factura A
                        if letra_fact not in ("A", "M") and fact["imp_iva"]:
                            subtotal += Decimal("{:.6f}".format(float(fact["imp_iva"])))
                        # mostrar descuento general solo si se utiliza:
                        if "descuento" in fact and fact["descuento"]:
                            descuento = Decimal("{:.6f}".format(float(fact["descuento"])))
                            f.set("descuento", self.fmt_imp(descuento))
                            subtotal -= descuento
                        # al subtotal neto sumo exento y no gravado:
                        if fact["imp_tot_conc"]:
                            subtotal += Decimal("{:.6f}".format(float(fact["imp_tot_conc"])))
                        if fact["imp_op_ex"]:
                            subtotal += Decimal("{:.6f}".format(float(fact["imp_op_ex"])))
                        ...
                        if letra_fact in ("A", "M"):
                            ...
                            for iva in fact["ivas"]:
                                ...
                                f.set(f"IVA{p}", self.fmt_imp(iva["importe"]))
                                f.set(f"NETO{p}", self.fmt_imp(iva["base_imp"]))
                                f.set(
                                    f"IVA{p}.L",
                                    "IVA {}".format(self.fmt_iva(iva["iva_id"])),
                                )
                        else:
            ...
            self.AgregarCampo(
                ...
                foreground=0xF00000,
                priority=-1,
                text="Excepcion {name}:{lineno}".format(**ex),
            )
            if DEBUG:
    ...
    @utils.inicializar_y_capturar_excepciones_simple
    def GenerarQR(self):
        from pyafipws.pdf.pyqr import PyQR
...
def main():
    ...
    if "--register" in sys.argv or "--unregister" in sys.argv:
        ...
        if "--prueba" in sys.argv:
            ...
            fepdf.EstablecerParametro("localidad_cliente", "Hurlingham")
            fepdf.EstablecerParametro("provincia_cliente", "Buenos Aires")
            ...
            if "--expo" in sys.argv:
                ...
                fepdf.AgregarPermiso(id_permiso, dst_merc)
