...
class PyQR:
    ...
    def CrearArchivo(self):
        ...
        tmp = tempfile.NamedTemporaryFile(
            prefix="qr_afip_", suffix=f".{self.Extension.lower()}", delete=False
        )
...
def main():
    ...
    if "--register" in sys.argv or "--unregister" in sys.argv:
        ...
        if "--prueba" in sys.argv:
            ...
            assert qr_data_test == qr_data_gen, f"Diff: {qr_data_test!r} != {qr_data_gen!r}"
            print("QR data ok:", qr_data_gen)

        if "--mostrar" not in sys.argv:
            pass
        elif sys.platform == "linux2" or sys.platform == "linux":
            os.system("eog " f"{pyqr.Archivo}")
        else:
