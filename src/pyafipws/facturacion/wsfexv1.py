...
from pyafipws.utils import BaseWS, get_install_dir, inicializar_y_capturar_excepciones
...
class WSFEXv1(BaseWS):
    ...
    def __analizar_errores(self, ret):
        ...
        if "FEXEvents" in ret:
            ...
            self.Eventos = [
                "{}: {}".format(evt["EventCode"], evt.get("EventMsg", "")) for evt in events
            ]
    ...
    def Authorize(self, id):
        ...
        if "FEXResultAuth" in result:
            ...
            self.FchVencCAE = vto
            self.Vencimiento = f"{vto[6:8]}/{vto[4:6]}/{vto[0:4]}"
            return self.CAE
    ...
    def GetParamUMed(self, sep="|"):
        ...
        if sep:
            return [
                ("\t{id}\t{ds}\t{vig_desde}\t{vig_hasta}\t".format(**it)).replace(
                    "\t", sep
                )
                for it in umeds
            ]
    ...
    def GetParamDstPais(self, sep="|"):
        ...
        for u in result["FEXResultGet"]:
            ...
            ret.append(r)
        if sep:
            return [("\t{codigo}\t{ds}\t".format(**it)).replace("\t", sep) for it in ret]
        else:
            return ret
    ...
    def GetParamTipoCbte(self, sep="|"):
        ...
        if sep:
            return [
                ("\t{codigo}\t{ds}\t{vig_desde}\t{vig_hasta}\t".format(**it)).replace(
                    "\t", sep
                )
                for it in ret
            ]
    ...
    def GetParamMonConCotizacion(self, fecha=None, sep="|"):
        ...
        if sep:
            return [
                ("\t{id}\t{ctz}\t{fecha}\t".format(**it)).replace("\t", sep)
                for it in mons
            ]
        else:
            return mons
    ...
    def GetParamPtosVenta(self, sep="|"):
        ...
        for pu in res:
            ...
            ret.append(r)
        return [
            ("{nro}\tBloqueado:{bloqueado}\tFchBaja:{baja}".format(**r)).replace(
                "\t", sep
            )
            for r in ret
        ]
    ...
    def GetParamActividades(self, sep="|"):
        ...
        if sep:
            return [
                ("\t{codigo}\t{ds}\t{orden}\t".format(**it)).replace("\t", sep)
                for it in ret
            ]
        else:
            return ret
...
class WSFEX(WSFEXv1):
    ...
    def __init__(self):
        ...
        self.Version = "{} {} WSFEXv1".format(__version__, HOMO and "Homologación" or "")
...
def main():
    ...
    #    win32com.server.localserver.serve([WSFEXv1._reg_clsid_])
        ...
        wsfexv1.Conectar(cache, wsdl, proxy, wrapper, cacert)
