...
class WSMTXCA(BaseWS):
    ...
    def inicializar(self):
        ...
        self.CbteNro = self.FechaCbte = None
    ...
    def AutorizarComprobante(self):
        ...
        fact = {
            ...
            "numeroComprobanteDesde": f["cbt_desde"],
            "numeroComprobanteHasta": f["cbt_hasta"],
        ...
        for error in ret.get("arrayObservaciones", []):
            self.Observaciones.append(
                "{codigo}: {descripcion}".format(**error["codigoDescripcion"])
            )
        self.Obs = "\n".join(self.Observaciones)

        if "evento" in ret:
            self.Evento = "{codigo}: {descripcion}".format(**ret["evento"])
        return self.CAE
    ...
    def ConsultarTiposComprobante(self):
        ...
        return [
            "{codigo}: {descripcion}".format(**p["codigoDescripcion"])
    ...
    def ConsultarPtosVtaCAEANoInformados(self, caea):
        ...
        return [
            " ".join([(f"{k}={v}") for k, v in list(p["puntoVenta"].items())])
    ...
    def ConsultarActividadesVigentes(self):
        ...
        return [
            "{codigo}: {orden} {descripcion}".format(**p["actividad"])
    ...
    def CondicionIvaReceptor(self, tipo_cbte=6, sep="|"):
        ...
        return ["{codigo}: {descripcion}".format(**p["codigoDescripcion"])
...
def main():
    ...
    if "--condicion-iva" in sys.argv:
        for tipo_cbte in 1, 2, 6:
            print(f"=== Condicion Iva Receptor {tipo_cbte} ===")
