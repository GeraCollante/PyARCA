...
from pyafipws.utils import BaseWS, get_install_dir, inicializar_y_capturar_excepciones, verifica
...
class WSFEv1(BaseWS):
    ...
    def __analizar_errores(self, ret):
        ...
        if "Events" in ret:
            events = ret["Events"]
            self.Eventos = [
                "{}: {}".format(evt["Evt"]["Code"], evt["Evt"]["Msg"]) for evt in events
            ]
    ...
    def ObtenerCampoFactura(self, *campos):
        ...
        for campo in campos:
            if isinstance(ret, dict) and isinstance(campo, basestring):
                ...
                self.Excepcion = f"El campo {campo} solicitado no existe"
                ret = None
            if ret is None:
                break
        return str(ret)
    ...
    def CAESolicitar(self):
        ...
        if "FeCabResp" in result:
            ...
            for obs in fedetresp.get("Observaciones", []):
                self.Observaciones.append("{Code}: {Msg}".format(**obs["Obs"]))
    ...
    def LeerFacturaX(self, i):
        ...
        try:
            ...
            for obs in f.get("obs", []):
                self.Observaciones.append("{code}: {msg}".format(**obs))
            self.Obs = "\n".join(self.Observaciones)
            return True
        except Exception:
            return False
    ...
    def ParamGetTiposCbte(self, sep="|"):
        ...
        return [
            ("{Id}\t{Desc}\t{FchDesde}\t{FchHasta}".format(**p["CbteTipo"])).replace(
                "\t", sep
            )
    ...
    def ParamGetTiposConcepto(self, sep="|"):
        ...
        return [
            (
                "{Id}\t{Desc}\t{FchDesde}\t{FchHasta}".format(**p["ConceptoTipo"])
            ).replace("\t", sep)
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetTiposDoc(self, sep="|"):
        ...
        return [
            ("{Id}\t{Desc}\t{FchDesde}\t{FchHasta}".format(**p["DocTipo"])).replace(
                "\t", sep
            )
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetTiposIva(self, sep="|"):
        ...
        return [
            ("{Id}\t{Desc}\t{FchDesde}\t{FchHasta}".format(**p["IvaTipo"])).replace(
                "\t", sep
            )
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetTiposMonedas(self, sep="|"):
        ...
        return [
            ("{Id}\t{Desc}\t{FchDesde}\t{FchHasta}".format(**p["Moneda"])).replace(
                "\t", sep
            )
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetTiposOpcional(self, sep="|"):
        ...
        return [
            (
                "{Id}\t{Desc}\t{FchDesde}\t{FchHasta}".format(**p["OpcionalTipo"])
            ).replace("\t", sep)
    ...
    def ParamGetTiposTributos(self, sep="|"):
        ...
        return [
            (
                "{Id}\t{Desc}\t{FchDesde}\t{FchHasta}".format(**p["TributoTipo"])
            ).replace("\t", sep)
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetTiposPaises(self, sep="|"):
        ...
        return [
            ("{Id}\t{Desc}".format(**p["PaisTipo"])).replace("\t", sep)
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetPtosVenta(self, sep="|"):
        ...
        return [
            (
                "{Nro}\tEmisionTipo:{EmisionTipo}\tBloqueado:{Bloqueado}\tFchBaja:{FchBaja}".format(**p["PtoVenta"])
            ).replace("\t", sep)
            for p in res.get("ResultGet", [])
        ]
    ...
    def ParamGetActividades(self, sep="|"):
        ...
        return [
            ("{Id}\t{Orden}\t{Desc}".format(**p["ActividadesTipo"])).replace("\t", sep)
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetCondicionIvaReceptor(self, clase_cmp="A", sep="|"):
        ...
        return [("{Id}\t{Desc}\t{Cmp_Clase}".format(**p['CondicionIvaReceptor'])).replace("\t", sep)
                 for p in res['ResultGet']]
...
def main():
    ...
    if "--parametros" in sys.argv:
        ...
        for clase_cmp in "A", "M", "B", "C":
            print(f"=== Condicion Iva Receptor {clase_cmp} ===")
    ...
    if "--sinmovimiento-caea" in sys.argv:
        ...
        if DEBUG:
            print(
                f"Informando Punto Venta {punto_vta} CAEA {caea} SIN MOVIMIENTO"
            )
