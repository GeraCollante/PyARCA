...
ENCABEZADO = [
    ...
    ("imp_total", 15, IMPORTE, "Importe total Double (13 + 2)"),
...
class WSCDC(BaseWS):
    ...
    def ConsultarModalidadComprobantes(self, sep="|"):
        ...
        self.__analizar_errores(result)
        return [
            ("\t{Cod}\t{Desc}\t".format(**p["FacModTipo"])).replace("\t", sep)
    ...
    def ConsultarTipoComprobantes(self, sep="|"):
        ...
        self.__analizar_errores(result)
        return [
            ("\t{Id}\t{Desc}\t".format(**p["CbteTipo"])).replace("\t", sep)
            for p in result["ResultGet"]
        ]
    ...
    def ConsultarTipoDocumentos(self, sep="|"):
        ...
        self.__analizar_errores(result)
        return [
            ("\t{Id}\t{Desc}\t".format(**p["DocTipo"])).replace("\t", sep)
            for p in result["ResultGet"]
        ]
    ...
    def ConsultarTipoOpcionales(self, sep="|"):
        ...
        self.__analizar_errores(result)
        return [
            ("\t{Id}\t{Desc}\t".format(**p["OpcionalTipo"])).replace("\t", sep)
            for p in res
        ]
...
def main():
    ...
    wscdc = WSCDC()
    wscdc.Conectar("", url_wscdc, proxy=proxy_dict)
