...
class WSFECred(BaseWS):
    ...
    def __analizar_errores(self, ret):
        ...
        self.ErrCode = " ".join(["{codigo}".format(**err) for err in errores])
        self.ErrMsg = "\n".join(
            ["{codigo}: {descripcion}".format(**err) for err in errores]
        )
    ...
    def __analizar_observaciones(self, ret):
        ...
        self.Obs = "\n".join(
            ["{codigo}: {descripcion}".format(**obs) for obs in self.Observaciones]
        )
    ...
    def __analizar_evento(self, ret):
        ...
        if evt:
            self.Eventos = [evt]
            self.Evento = "{codigo}: {descripcion}".format(**evt)
    ...
    def ConsultarTiposAjustesOperacion(self, sep="||"):
        ...
        return [
            (f"{sep} {{codigo}} {sep} {{descripcion}} {sep}").format(**it)
            if sep
            else it
            for it in lista
        ]
    ...
    def ConsultarTiposRetenciones(self, sep="||"):
        ...
        return [
            (
                f"{sep} {{codigoJurisdiccion}} {sep} {{descripcionJurisdiccion}} {sep} {{porcentajeRetencion}} {sep}"
            ).format(**it)
            if sep
            else it
            for it in lista
...
FORMATOS = {
    "encabezado": [
        ...
        ("nro_cbte", 8, N),
        ("cod_moneda", 3, A),
        ("ctz_moneda_ult", 18, IMPORTE, 6),
        ("importe_cancelado", 17, IMPORTE, 2),
        ("importe_embargo_pesos", 17, IMPORTE, 2),
        ("importe_total_ret_pesos", 17, IMPORTE, 2),
        ("saldo_aceptado", 17, IMPORTE, 2),
    ...
    "cta_cte": [
        ...
        ("punto_vta", 11, N),
        ("nro_cbte", 8, N),
        ("importe_total_fc", 19, IMPORTE, 2),
        ("importe_cancelado", 19, IMPORTE, 2),
        ("importe_total_ndc", 19, IMPORTE, 2),
        ("importe_tot_ret_pes", 19, IMPORTE, 2),
        ("importe_embargo_pes", 19, IMPORTE, 2),
        ("saldo_aceptado", 19, IMPORTE, 2),
        ("saldo", 19, IMPORTE, 2),
        ("cod_moneda", 3, A),
        ("ctz_moneda_ult", 18, IMPORTE, 6),
    ],
    "comprobante": [
        ...
        ("punto_vta", 11, N),
        ("cbte_nro", 8, N),
        ("imp_total", 19, IMPORTE, 2),
        ("moneda_id", 3, A),
        ("moneda_ctz", 18, IMPORTE, 6),
    ...
    try:
        ...
        if "--ctasctes" in sys.argv:
            try:
                ...
                raise RuntimeError(f"Revise los parámetros: {ex}")
