...
class WSCOC:
    ...
    def Conectar(
        ...
        if timeout:
            self.__log(f"Estableciendo timeout={timeout}")
        ...
        self.__log(f"Conectando a wsdl={wsdl} cache={cache} proxy={proxy_dict}")
        ...
        self.__log(f"Corrigiendo location={location}")
        return True
    ...
    def ConsultarCUIT(self, numero_doc, tipo_doc=80, sep="|"):
        ...
        if "consultarCUITReturn" in res:
            ...
            if "tipoNumeroDoc" in ret:
                ...
                return [
                    ("{cuit}\t{denominacion}".format(**d["detalleCUIT"])).replace(
                        "\t", sep
                    )
    ...
    def ConsultarMonedas(self, sep="|"):
        ...
        return [
            ("{codigo}\t{descripcion}".format(**p["codigoDescripcion"])).replace("\t", sep)
    ...
    def ConsultarTiposEstadoSolicitud(self, sep="|"):
        ...
        return [
            ("{codigo}\t{descripcion}".format(**p["codigoDescripcionString"])).replace(
                "\t", sep
            )
...
def main():
    ...
    if "--parametros" in sys.argv:
        ...
        print(
            "\n".join(
                [f"||{s}||" for s in ws.ConsultarTiposEstadoSolicitud(sep="||")]
            )
        )
        print("=== Monedas ===")
        print("\n".join([f"||{s}||" for s in ws.ConsultarMonedas(sep="||")]))
        print("=== Destinos de Compra ===")
        print("\n".join([f"||{s}||" for s in ws.ConsultarDestinosCompra(sep="||")]))
        print("=== Tipos de Documento ===")
        print("\n".join([f"||{s}||" for s in ws.ConsultarTiposDocumento(sep="||")]))
        ...
        print(
            "\n".join(
                [f"||{s}||" for s in ws.ConsultarMotivosExcepcionDJAI(sep="||")]
            )
        )
        print("=== Destinos Compra DJAI ===")
        print(
            "\n".join([f"||{s}||" for s in ws.ConsultarDestinosCompraDJAI(sep="||")])
        )
        print("=== Motivos Excepcion DJAS ===")
        print(
            "\n".join(
                [f"||{s}||" for s in ws.ConsultarMotivosExcepcionDJAS(sep="||")]
            )
        )
        print("=== Destinos Compra DJAS ===")
        print(
            "\n".join([f"||{s}||" for s in ws.ConsultarDestinosCompraDJAS(sep="||")])
        )
