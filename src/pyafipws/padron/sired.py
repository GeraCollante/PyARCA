...
from builtins import input, str, zip
...
from pyafipws.utils import IMPORTE, A, B, C, N, escribir, get_install_dir, leer
...
CAB_FAC_TIPO1 = [
    ...
    ("nro_doc", 11, N),
    ("nombre", 30, A),
    ("imp_total", 15, IMPORTE),
    ("imp_tot_conc", 15, IMPORTE),
    ("imp_neto", 15, IMPORTE),
    ("impto_liq", 15, IMPORTE),
    ("impto_liq_rni", 15, IMPORTE),
    ("imp_op_ex", 15, IMPORTE),
    ("impto_perc", 15, IMPORTE),
    ("imp_iibb", 15, IMPORTE),
    ("impto_perc_mun", 15, IMPORTE),
    ("imp_internos", 15, IMPORTE),
    ("transporte", 15, IMPORTE),
    ("categoria", 2, N),
    ("imp_moneda_id", 3, A),
    ("imp_moneda_ctz", 10, IMPORTE),
    ("alicuotas_iva", 1, N),
...
DETALLE = [
    ...
    ("qty", 12, IMPORTE),
    ("pro_umed", 2, N),
    ("pro_precio_uni", 16, IMPORTE),
    ("imp_bonif", 15, IMPORTE),
    ("imp_ajuste", 16, IMPORTE),
    ("imp_total", 16, IMPORTE),
    ("alicuota_iva", 4, IMPORTE),
    ("gravado", 1, C),
    ("anulacion", 1, C),
    ("codigo", 50, A),
    ("ds", 150, A),
...
from pyafipws.provincial.rg3685 import REGINFO_CV_VENTAS_CBTE
...
def leer_planilla(entrada, sep=","):
    ...
    if len(items[0]) < 2:
        raise RuntimeError(f"El archivo no tiene columnas (usar {sep} de separador)")
...
def grabar_json(items, salida="sired.json"):
...
def generar_encabezado(items):
    ...
    out = open(f"CABECERA_{periodo}.txt", "w")
...
def generar_detalle(items):
    ...
    out = open(f"DETALLE_{periodo}.txt", "w")
...
def generar_ventas(items):
    ...
    out = open(f"VENTAS_{periodo}.txt", "w")
    ...
    for item in items:
        ...
        for i, iva in enumerate(ivas):
            ...
            for k, _len, t in VENTAS_TIPO1[1:10] + VENTAS_TIPO1[21:30]:
                vals[k] = item.get(k)
            vals["fecha_anulacion"] = ""
            vals["ctl_fiscal"] = item.get("ctl_fiscal", " ")  # C para controlador
            vals["anulacion"] = item.get("anulacion", " ")
...
def main():
    ...
    try:
        ...
        if "--prueba" in sys.argv:
            ...
            sired.ObtenerFactura(id_factura)
            f = sired.factura
        ...
        if "--leer" in sys.argv:
            if "--completar_padron" in sys.argv:
                from pyafipws.padron.padron import PadronAFIP
            ...
            csv_writer.writerow(dict([(k, k) for k in claves]))
            f = open("VENTAS.txt")
            for linea in f:
                if str(linea[0]) == "2":
                    ...
                    if "--completar_padron" in sys.argv:
                        ...
                        datos["localidad_cliente"] = "{} (CP {}) ".format(
