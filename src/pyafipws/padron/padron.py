...
class PadronAFIP:
    ...
    def Procesar(self, filename="padron.txt", borrar=False):
        ...
        f = open(filename)
        keys = [k for k, _len, t, d in FORMATO]
        ...
        if True:
            ...
            for i, line in enumerate(f):
                ...
                c.execute(f"INSERT INTO padron VALUES ({placeholders})", params)
            db.commit()
            c.close()
            db.close()
    ...
    def Buscar(self, nro_doc, tipo_doc=80):
        ...
        for key in [k for k, _len, t, d in FORMATO]:
            if row:
                val = row[key]
    ...
    def BuscarCUIT(self, denominacion, limite=10):
        ...
        self.cursor.execute(sql, [f"*{denominacion}*", limite])
        filas = self.cursor.fetchall()
    ...
    def Consultar(self, nro_doc):
        ...
        if result["success"]:
            ...
            # retrocompatibilidad:
            self.domicilios = [
                f"{self.direccion} - {self.localidad} ({self.cod_postal}) - {self.provincia}"
            ]
            # analizo impuestos:
    ...
    def ObtenerTablaParametros(self, tipo_recurso, sep="||"):
        ...
        if result["success"]:
            ...
            self.Excepcion = error["mensaje"]
        if sep:
            return [f"{sep}%s{sep}%s{sep}" % it for it in sorted(ret.items())]
        else:
            return ret
    ...
    if "--register" in sys.argv or "--unregister" in sys.argv:
        ...
        if "--csv" in sys.argv:
            csv_reader = csv.reader(
                open("entrada.csv"), dialect="excel", delimiter=","
            )
