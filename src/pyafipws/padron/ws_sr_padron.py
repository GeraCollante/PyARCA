...
from pyafipws.padron.padron import PROVINCIAS, TIPO_CLAVE
...
class WSSrPadronA4(BaseWS):
    ...
    def Consultar(self, id_persona):
        ...
        self.domicilio = f"{self.direccion} - {self.localidad} ({self.cod_postal}) - {self.provincia}"
        # analizo impuestos:
        self.impuestos = [
            imp["idImpuesto"]
...
def main():
    ...
    if "--csv" in sys.argv:
        csv_reader = csv.reader(
            open("tests/entrada.csv"), dialect="excel", delimiter=","
        )
    ...
    try:
        ...
        if "--testing" in sys.argv:
            padron.LoadTestXML(f"tests/xml/{service}_resp.xml")
