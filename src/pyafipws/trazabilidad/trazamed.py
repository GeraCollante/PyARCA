...
class TrazaMed(BaseWS):
    ...
    def __analizar_errores(self, ret):
        ...
        self.Errores = [
            "{}: {}".format(it["_c_error"], it["_d_error"]) for it in ret.get("errores", [])
        ]
...
def main():
    ...
    if "--cargar" in sys.argv:
        if "--dbf" in sys.argv:
            ...
            for formato in formatos[:1]:
                archivo = open(formato[0].lower() + ".json")
                d = json.load(archivo)
                formato[2].extend(d)
                archivo.close()
        else:
            for formato, campos, lista in formatos[:1]:
                archivo = open(formato.lower() + ".txt")
    ...
    if "--cancela" in sys.argv:
        ...
        ws.GetCatalogoElectronicoByGTIN(
        ...
        ws.GetConsultaStock(*sys.argv[sys.argv.index("--stock") + 1 :])
