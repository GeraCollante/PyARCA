...
class WSCPE(BaseWS):
    ...
    def __analizar_errores(self, ret):
        ...
        self.Errores = ["{codigo}: {descripcion}".format(**err) for err in errores]
        self.ErrCode = " ".join(["{codigo}".format(**err) for err in errores])
        self.ErrMsg = "\n".join(["{codigo} - {descripcion}".format(**err) for err in errores])
    ...
    def __analizar_observaciones(self, ret):
        ...
        self.Obs = "\n".join(["{codigo}: {descripcion}".format(**obs) for obs in self.Observaciones])
    ...
    def EditarCPEConfirmadaFerroviariaDg(
        ...
        **kwargs
        ...
        """Modificar datos de una CP Ferroviaria Derivados Granarios."""
        self.cpe.update({
            "nroCTG": nro_ctg,
            "observaciones": observaciones,
        })
    ...
    def ConsultarCPEPorDestino(
        ...
        if "cartaPorte" in ret:
            ...
            return [
                (f"{sep} {{nroCTG}} {sep} {{fechaPartida}} {sep} {{estado}} {sep} {{fechaUltimaModificacion}} {sep}").format(**it)
    ...
    def ConsultarProvincias(self, sep="||"):
        ...
        return [(f"{sep} {{codigo}} {sep} {{descripcion}} {sep}").format(**it) if sep else it for it in array]
    ...
    def ConsultarPlantas(self, cuit, sep="||"):
        ...
        if "planta" in ret:
            ...
            return [
                (f"{sep} {{nroPlanta}} {sep} {{codProvincia}} {sep} {{codLocalidad}} {sep}").format(**it)
                if sep else it for it in array
            ]
