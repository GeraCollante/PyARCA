...
from pyafipws.remitos.wscpe import HOMO, WSCPE

# importo funciones compartidas:
from pyafipws.utils import IMPORTE, A, B, N, exception_info, grabar_txt, guardar_dbf, json, leer_dbf, leer_txt
        ...
        for msg, formato in sorted(FORMATOS.items(), key=lambda x: TIPO_REGISTROS_REV[x[0]]):
            ...
            for fmt in formato:
                ...
                print("|| %-45s || %8d || %8d || %-12s || %-4s || %-5s ||" % (
                    ...
                    (f"{dec}") if tipo == IMPORTE else "",
                    (f"{tipo_reg}") if clave == "tipo_reg" else "",
                ))
                comienzo += longitud
        sys.exit(0)
    ...
    try:
        ...
        if '--consultar' in sys.argv:
            ...
            if wscpe.Excepcion:
                print ("EXCEPCION:", wscpe.Excepcion, file=sys.stderr)
                if DEBUG:
                    print(wscpe.Traceback, file=sys.stderr)
        ...
        if '--autorizar' in sys.argv:
            ...
            if "--ferroviaria" not in sys.argv:
