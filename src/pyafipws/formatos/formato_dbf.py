...
def definir_campos(formato):
    ...
    for fmt in formato:
        ...
        if longitud > 250:
            ...
            tipo = f"C({longitud})"  # character
        elif tipo == N:
            tipo = f"N({longitud},0)"  # numeric
        elif tipo == IMPORTE:
            tipo = f"N({longitud},{decimales})"  # "currency"
        else:
            raise RuntimeError(
                f"Tipo desconocido: {tipo} {clave} {longitud} {decimales}"
            )
...
def leer(archivos=None, carpeta=None):
    ...
    for nombre, formato, subclave in formatos:
        filename = archivos.get(nombre.lower(), f"{nombre[:8]}.dbf").strip()
        if not filename:
            continue
...
def escribir(regs, archivos=None, carpeta=None):
    ...
    for reg in regs:
        ...
        for nombre, formato, registros in formatos:
            ...
            filename = archivos.get(nombre.lower(), f"{nombre[:8]}.dbf")
            # construir ruta absoluta si se especifica carpeta
            if carpeta is not None:
                filename = os.path.join(carpeta, filename)
            if DEBUG:
