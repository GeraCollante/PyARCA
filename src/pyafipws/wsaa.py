        ...
        csrs = self.x509_req.subject_name(
            x509.Name(
                [
                    ...
                    x509.NameAttribute(NameOID.ORGANIZATION_NAME, f"{empresa}"),
                    x509.NameAttribute(NameOID.COMMON_NAME, f"{nombre}"),
        ...
        try:
            ...
            for filename in (crt, key):
                if not os.access(filename, os.R_OK):
                    raise RuntimeError(f"Imposible abrir {filename}\n")
            # creo el nombre para el archivo del TA (según credenciales y ws)
            fn = (
                "TA-{}.xml".format(hashlib.md5((service + os.path.abspath(crt) + os.path.abspath(key)).encode("utf8")).hexdigest())
            )
            if cache:
            ...
            if (
                ...
                if not ok or self.Excepcion:
                    raise RuntimeError(f"Fallo la conexión: {self.Excepcion}")
                # llamar al método remoto para solicitar el TA
                if DEBUG:
                    print("Llamando WSAA...")
                ta = self.LoginCMS(cms)
                if not ta:
                    raise RuntimeError(f"Ticket de acceso vacio: {WSAA.Excepcion}")
                # grabar el ticket de acceso para poder reutilizarlo luego
                if DEBUG:
                    print(f"Grabando TA en {fn}...")
                try:
                    open(fn, "w").write(ta)
                except OSError:
                    self.Excepcion = f"Imposible grabar ticket de accesso: {fn}"
            else:
                # leer el ticket de acceso del archivo en cache
                if DEBUG:
                    print(f"Leyendo TA de {fn}...")
                ta = open(fn).read()
    ...
    if "--register" in sys.argv or "--unregister" in sys.argv:
        ...
        if not empresa:
            ...
            if ok and padron.denominacion:
                ...
                print(f"CUIT {cuit} no encontrado: {padron.Excepcion}...")
        ...
        clave_privada = f"clave_privada_{cuit}_{ts}.key"
        pedido_cert = f"pedido_cert_{cuit}_{ts}.csr"
        print(f"Longitud clave {key_length} (bits)")
        ...
        print(
            f"Usando CRT={crt} KEY={key} URL={url} SERVICE={service} TTL={ttl}",
            file=sys.stderr,
        )
        ...
        print(f"WSAA Version {WSAA.Version} {HOMO}", file=sys.stderr)
        ...
        if not ta:
            ...
            sys.exit(f"Excepcion: {wsaa.Excepcion}")
