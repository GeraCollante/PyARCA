...
from pyafipws.facturacion import wsct
...
def depurar_xml(client):
    ...
    f = open(os.path.join(wsct_xml_dir, f"request-{fecha}.xml"), "w")
    f.write(client.xml_request)
    f.close()
    f = open(os.path.join(wsct_xml_dir, f"response-{fecha}.xml"), "w")
