...
from pyafipws.facturacion import wsfev1
...
def autorizar(ws, entrada, salida, informar_caea=False):
    ...
    if "/dbf" in sys.argv:
        ...
        for linea in entrada:
            if str(linea[0]) == "0":
                ...
                if DEBUG:
                    print(len(encabezados), "Leida factura {cbt_desde}".format(**encabezado))
            elif str(linea[0]) == "1":
    ...
    for encabezado in encabezados:
        ...
        if DEBUG:
            print(
                "\n".join(
                    [f"{k}='{str(v)}'" for k, v in list(ws.factura.items())]
                )
            )
...
def depurar_xml(client, ruta="."):
    if XML:
        ...
        f = open(os.path.join(ruta, f"request-{fecha}.xml"), "w")
        f.write(client.xml_request)
        f.close()
        f = open(os.path.join(ruta, f"response-{fecha}.xml"), "w")
        f.write(client.xml_response)
        f.close()


def main():
    global HOMO, DEBUG, CONFIG_FILE, TIMEOUT, XML, conf_dbf
    ...
    if config.has_section("PROXY") and not HOMO:
        proxy_dict = dict((f"proxy_{k}", v) for k, v in config.items("PROXY"))
