...
from pyafipws.facturacion import wsmtx
from pyafipws.utils import IMPORTE, A, N, SoapFault, abrir_conf, escribir, guardar_dbf, leer, leer_dbf
...
if "--pyfepdf" not in sys.argv:
    ...
    ENCABEZADO = [
        ...
        ("imp_total", 15, IMPORTE, 2),
        ("imp_tot_conc", 15, IMPORTE, 2),
        ("imp_neto", 15, IMPORTE, 2),
        ("imp_subtotal", 15, IMPORTE, 2),
        ("imp_trib", 15, IMPORTE, 2),
        ("imp_op_ex", 15, IMPORTE, 2),
        ("moneda_id", 3, A),
        ("moneda_ctz", 10, IMPORTE, 6),  # 10,6
    ...
    DETALLE = [
        ...
        ("cod_mtx", 30, A),
        ("codigo", 30, A),
        ("qty", 15, IMPORTE, 3),  # debería ser 18,6 pero el DBF no lo soporta
        ("umed", 3, N),
        ("precio", 15, IMPORTE, 3),  # debería ser 18,6 pero el DBF no lo soporta
        ("bonif", 15, IMPORTE, 3),
        ("iva_id", 3, N),
        ("imp_iva", 15, IMPORTE, 2),
        ("imp_subtotal", 15, IMPORTE, 2),
        ("ds", 4000, A),
    ]
    ...
    TRIBUTO = [
        ...
        ("base_imp", 15, IMPORTE, 2),
        ("alic", 15, IMPORTE, 2),  # no se usa...
        ("importe", 15, IMPORTE, 2),
    ]

    IVA = [
        ("tipo_reg", 1, N),  # 2: IVA
        ("iva_id", 3, A),  # código de alícuota
        ("base_imp", 15, IMPORTE, 2),  # no se usa...
        ("importe", 15, IMPORTE, 2),
    ]
...
def autorizar(ws, entrada, salida, informar_caea=False):
    ...
    if DEBUG:
        print("\n".join([f"{k}='{str(v)}'" for k, v in list(ws.factura.items())]))
    if not DEBUG :
...
def depurar_xml(client):
    ...
    f = open(f"request-{fecha}.xml", "w")
    f.write(client.xml_request)
    f.close()
    f = open(f"response-{fecha}.xml", "w")
    f.write(client.xml_response.decode())
    f.close()


def main():
    global HOMO, DEBUG, XML, CONFIG_FILE, conf_dbf
    ...
    if DEBUG:
        print(f"wsaa_url {wsaa_url}\nwsmtxca_url {wsmtxca_url}\ncuit {cuit}")
    ...
    try:
        ...
        if "/solicitarcaea" in sys.argv:
            ...
            if DEBUG:
                print(f"Solicitando CAEA para periodo {periodo} orden {orden}")
            ...
            if not caea:
                if DEBUG:
                    print(
                        f"Consultando CAEA para periodo {periodo} orden {orden}"
                    )
        ...
        if "/consultarcaea" in sys.argv:
            ...
            if DEBUG:
                print(f"Consultando CAEA para periodo {periodo} orden {orden}")
        ...
        if "/informarcaeanoutilizado" in sys.argv:
            ...
            if i + 1 < len(sys.argv):
                ...
                caea = input("CAEA: ")
            if DEBUG:
                print(f"Informando CAEA no utilizado: {caea}")
        ...
        if "/informarcaeanoutilizadoptovta" in sys.argv:
            ...
            if DEBUG:
                print(f"Informando CAEA no utilizado: {caea} pto_vta {pto_vta}")
        ...
        if "/consultarptosvtacaeanoinformados" in sys.argv:
            ...
            if i + 1 < len(sys.argv):
                ...
                caea = input("CAEA: ")
            if DEBUG:
                print(f"Consultando PtosVta CAEA: {caea}")
