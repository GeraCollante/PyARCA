...
from pyafipws.facturacion import wsfexv1
from pyafipws.utils import IMPORTE, A, N, abrir_conf, escribir, guardar_dbf, leer, leer_dbf
...
if '--pyfepdf' not in sys.argv:
    ...
    from formato_txt import CMP_ASOC, DETALLE, ENCABEZADO, PERMISO

    TIPOS_REG = "0", "1", "2", "3"

if "/recex" in sys.argv:
    from recex import CMP_ASOC, DETALLE, ENCABEZADO, PERMISO

    ENCABEZADO[8] = ("nombre_cliente", 200, A)  # 'Joao Da Silva'
    ENCABEZADO[7] = ("pais_dst_cmp", 3, N)
    ENCABEZADO[16] = ("obs_generales", 1000, A)
    DETALLE[5] = ("importe", 13, IMPORTE, 2)
    DETALLE.append(("bonif", 12, IMPORTE, 6))
...
def autorizar(ws, entrada, salida):
    ...
    if DEBUG:
        # print f.to_dict()
        print("\n".join([f"{k}='{str(v)}'" for k, v in list(encabezado.items())]))
        for detalle in detalles:
            print(
                ", ".join([f"{k}='{str(v)}'" for k, v in list(detalle.items())])
            )
...
def main():
    global HOMO, DEBUG, XML, CONFIG_FILE, TIMEOUT, conf_dbf
