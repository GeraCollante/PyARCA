...
from pyafipws.utils import abrir_conf, guardar_dbf, leer_dbf
...
from pyafipws.utils import ALFANUMERICO as A
from pyafipws.utils import IMPORTE
from pyafipws.utils import NUMERICO as N  # noqa: E402
...
def autorizar(ws, entrada, salida):
    ...
    if "testing" in sys.argv:
        ult_cbte = ws.GetLastCMP(encabezado["punto_vta"], encabezado["tipo_cbte"])
    ...
    if DEBUG:
        print("\n".join([f"{k}='{v}'" for k, v in list(ws.factura.items())]))
        print("id:", encabezado["id"])
    if not DEBUG :
        ws.Authorize(encabezado["id"])
        dic = ws.factura
        dic.update(
            {
                "id": encabezado["id"],
...
def main():
    global HOMO, DEBUG, XML, CONFIG_FILE, wsaa, conf_dbf
